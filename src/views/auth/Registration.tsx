...
import { RegistrationData, RegistrationFormElement } from '../../interfaces/signInInterfaces';
...
function Registration() {
  ...
  const handleRegistration = (event: React.FormEvent<RegistrationFormElement>) => {
    ...
    registrationSchema
      ...
      .then((result) => {
        if (result.password !== result.repeatPassword) {
          ...
          result.phone.toString().length !== 9 &&
          !matchPhoneCodes(result.phone.toString().padStart(10, '0'))
        ) {
  ...
  return (
    <div className="container my-3">
      ...
      <form className="row mt-4" onSubmit={handleRegistration}>
        ...
        <InputSignIn placeholder={'סיסמה'} type={'password'} id={'password'} wrong={wrongPass} />
        <InputSignIn
          placeholder={'הזנה'}
          type={'password'}
          id={'repeatPassword'}
